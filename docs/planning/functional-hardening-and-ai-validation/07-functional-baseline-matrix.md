...
| 项目 | 当前结论 | 证据 |
|---|---|---|
| 构建基线 | `npm run build` 于 2026-03-30 通过。 | 当前工作区在 `R5-AI-08` settings diagnostics UI 收口后执行通过。 |
| 自动化测试基线 | `npm test` 于 2026-03-30 通过，`19` 个测试文件、`154` 条测试全部通过。 | 当前工作区在 `R5-AI-08` settings diagnostics UI 收口后执行通过。 |
...
| AI test 瞬时重试 gate | `/api/ai/test` 现在会对 timeout / 连接型故障重试 1 次；本地合同已通过，且在重试后仍失败时会返回 `/models` 诊断信息；当前本地 provider 的 focused H1 replay 仍未恢复绿色。 | [AI test 瞬时重试验收记录](./26-ai-test-retry-validation.md)、[AI provider 直连诊断验收记录](./27-ai-provider-diagnostic-validation.md)。 |
| AI provider 直连诊断 gate | `scripts/ai-provider-diagnose.ts` 已加入；2026-03-30 当前本地 provider 的直连结果是 `/models = 200`、`modelFound = true`、`modelCount = 11`，但 `/chat/completions` 在 `30s` timeout 窗口内仍失败。 | [AI provider 直连诊断验收记录](./27-ai-provider-diagnostic-validation.md)。 |
...
| 真实 AI gate | `R15-H1-04` 的历史 3 样本联调已通过 organize apply / rollback；但当前本地 provider 在 2026-03-30 的 focused replay 上仍未恢复绿色，且 direct diagnose 已确认主要问题位于 chat completion 链路，因此 real-provider gate 只可视为“历史基线 + 当前需复核”。 | [真实 AI 提供方联调与人工验收](./10-ai-provider-h1-validation.md)、[单条 classify H1 语义复验记录](./24-single-classify-h1-replay-validation.md)、[AI provider 直连诊断验收记录](./27-ai-provider-diagnostic-validation.md)。 |
...
| AI classify / test / classify-batch | `src/routes/ai.ts`、`src/ai-classify-guardrail.ts`、`scripts/ai-classify-semantic-validate.ts`、`scripts/ai-h1-classify-semantic-validate.ts`、`scripts/ai-provider-diagnose.ts`、`scripts/settings-ai-diagnostic-validate.ts` | `tests/ai-classify-guardrail.test.ts`、`tests/ai-classify-semantic-samples.test.ts`、`tests/integration/ai-routes.test.ts`、`tests/integration/ai-harness.test.ts`、`tests/integration/page-assets.test.ts` | `R15-H1-04` 已做历史真实 provider 验收；`R5-AI-04` 又补了 focused H1 replay；`R5-AI-05` 再补单条 classify timeout fallback；`R5-AI-06` 再补 `/api/ai/test` 瞬时重试；`R5-AI-07` 再补 direct diagnose 与 timeout diagnostic payload；`R5-AI-08` 再把诊断结果接到设置页 UI | 输出合同、常见语义漂移和固定语义样本集都已收口；当前剩余主要缺口已精确收敛到 real provider 的 chat completion 可用性 / timeout 稳定性，而不是配置探活本身或 UI 可见性不足 | `R15-AI-01`、`R15-AI-02`、`R15-H1-04`、`R5-AI-01`、`R5-AI-02`、`R5-AI-03`、`R5-AI-04`、`R5-AI-05`、`R5-AI-06`、`R5-AI-07`、`R5-AI-08` |
...
- `R1.5` 的 AI gate 若在发版前沿用当前 provider / model，还必须额外复跑 [24-single-classify-h1-replay-validation.md](./24-single-classify-h1-replay-validation.md)、[25-single-classify-timeout-fallback-validation.md](./25-single-classify-timeout-fallback-validation.md)、[26-ai-test-retry-validation.md](./26-ai-test-retry-validation.md)、[27-ai-provider-diagnostic-validation.md](./27-ai-provider-diagnostic-validation.md) 和 [28-settings-ai-diagnostic-ui-validation.md](./28-settings-ai-diagnostic-ui-validation.md) 对应脚本 / 结论，确认 `/api/ai/test` 状态、单条 classify fallback、direct diagnose 和设置页诊断 UI 都在预期内。
...
| AI classify / test / classify-batch | `tests/ai-classify-guardrail.test.ts`、`tests/ai-classify-semantic-samples.test.ts`、`tests/integration/ai-routes.test.ts`、`tests/integration/ai-harness.test.ts`、`tests/integration/page-assets.test.ts` + `scripts/ai-classify-semantic-validate.ts` + `scripts/ai-h1-classify-semantic-validate.ts` + `scripts/ai-provider-diagnose.ts` + `scripts/settings-ai-diagnostic-validate.ts` + H1 实测 | 本地合同已纳入回归；单条 classify 的 timeout fallback 已在真实 provider 下恢复 `1/1` focused replay；`/api/ai/test` 现在还能在失败时给出 `/models` 级别诊断，设置页也能直接显示这组信息 | 单条 `/api/ai/classify` 已同时具备 taxonomy guardrail、常见内容信号语义择优、固定语义样本集和 timeout fallback；截至 2026-03-30，当前本地 provider 的主要残余已进一步收敛到 chat completion 链路的可用性，而不是单条 classify 高信号输入的完全不可用，也不是 base_url / model 配置本身不可达或 UI 完全不可见。 |

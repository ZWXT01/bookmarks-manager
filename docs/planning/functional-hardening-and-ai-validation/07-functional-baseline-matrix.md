...
| 项目 | 当前结论 | 证据 |
|---|---|---|
| 构建基线 | `npm run build` 于 2026-03-30 通过。 | 当前工作区在 `R6-AI-01` organize apply 合同收口后重新 clean run 通过。 |
| 自动化测试基线 | `npm test` 于 2026-03-30 通过，`21` 个测试文件、`164` 条测试全部通过。 | 当前工作区在 `R6-UI-02` 模板弹窗可达性收口后重新 clean run 通过。 |
...
| 模板 | `src/routes/templates.ts`、`src/template-service.ts`、`views/index.ejs`、`public/app.js` | `tests/integration/ops-routes.test.ts`、`tests/integration/page-assets.test.ts` | `scripts/category-interaction-validate.ts` 已覆盖模板切换；`scripts/template-editor-validate.ts` 已覆盖长树编辑与 footer 可达性 | 剩余缺口主要在“模板编辑后默认 AI 入口统一读最新活动模板”与“扩充更多预置模板” | `R1-API-02`、`R6-UI-02`、`R6-AI-03`、`R6-TPL-06` |
...
| AI classify / test / classify-batch | `src/routes/ai.ts`、`src/ai-classify-guardrail.ts`、`src/ai-client.ts`、`src/provider-validation-config.ts`、`scripts/ai-classify-semantic-validate.ts`、`scripts/ai-h1-classify-semantic-validate.ts`、`scripts/ai-provider-diagnose.ts`、`scripts/settings-ai-diagnostic-validate.ts` | `tests/ai-client.test.ts`、`tests/ai-classify-guardrail.test.ts`、`tests/ai-classify-semantic-samples.test.ts`、`tests/provider-validation-config.test.ts`、`tests/integration/ai-routes.test.ts`、`tests/integration/ai-harness.test.ts`、`tests/integration/page-assets.test.ts` | `R15-H1-04` 已做历史真实 provider 验收；`R5-AI-04` 到 `R5-AI-08` 逐步补齐 focused H1、timeout fallback、重试、direct diagnose 与设置页诊断；`R5-AI-09` 又把默认 provider 验证源收口为 Grok，并兼容 SSE completion | 输出合同、常见语义漂移、固定语义样本集、provider diagnose 和真实 H1 现在都已闭环；当前主要要求变成“若手工切换 provider 或 endpoint，必须复跑默认 Grok 之外的当前源验证”，而不是“默认验证源本身不稳定” | `R15-AI-01`、`R15-AI-02`、`R15-H1-04`、`R5-AI-01`、`R5-AI-02`、`R5-AI-03`、`R5-AI-04`、`R5-AI-05`、`R5-AI-06`、`R5-AI-07`、`R5-AI-08`、`R5-AI-09` |
| AI organize 生命周期 | `src/routes/ai.ts`、`src/ai-organize.ts`、`src/ai-organize-plan.ts`、`src/routes/pages.ts` | `tests/ai-organize-plan.test.ts`、`tests/integration/ai-organize-routes.test.ts` | 任务详情页可展示 organize 计划；MCP UI 与真实 H1 已覆盖 apply / rollback | 多待应用 plan 合同已收口；剩余缺口主要在“模板编辑后默认入口统一读最新活动模板” | `R15-AI-03`、`R3-AI-02`、`R6-AI-01`、`R6-AI-03` |
...
| 模板弹窗 UI 可达性 | `tests/integration/page-assets.test.ts` + `scripts/template-editor-validate.ts` + [31-template-editor-modal-validation.md](./31-template-editor-modal-validation.md) | 已纳入回归 | 当前已验证模板选择 / 编辑弹窗在长树、小视口下的保存 / 取消可达性；剩余模板相关工作转入 `R6-AI-03` 与 `R6-TPL-06`。 |

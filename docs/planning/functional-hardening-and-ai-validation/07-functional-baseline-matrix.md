...
| 项目 | 当前结论 | 证据 |
|---|---|---|
| 构建基线 | `npm run build` 于 2026-03-30 通过。 | 当前工作区在 `R5-AI-04` focused replay 回填后执行通过。 |
| 自动化测试基线 | `npm test` 于 2026-03-30 通过，`19` 个测试文件、`152` 条测试全部通过。 | 当前工作区在 `R5-AI-04` focused replay 回填后执行通过。 |
...
| 单条 classify H1 focused replay | `scripts/ai-h1-classify-semantic-validate.ts` 已加入；但 2026-03-30 对当前本地 provider 做 focused replay 时，`/api/ai/test` 与单条 classify 都返回 `Request timed out.`。 | [单条 classify H1 语义复验记录](./24-single-classify-h1-replay-validation.md)。 |
...
| AI classify / test / classify-batch | `src/routes/ai.ts`、`src/ai-classify-guardrail.ts`、`scripts/ai-classify-semantic-validate.ts`、`scripts/ai-h1-classify-semantic-validate.ts` | `tests/ai-classify-guardrail.test.ts`、`tests/ai-classify-semantic-samples.test.ts`、`tests/integration/ai-routes.test.ts`、`tests/integration/ai-harness.test.ts` | `R15-H1-04` 已做历史真实 provider 验收；`R5-AI-04` 又补了 focused H1 replay | 输出合同、常见语义漂移和固定语义样本集都已收口；当前剩余主要缺口不是本地 guardrail，而是 real provider 的可用性 / timeout 稳定性 | `R15-AI-01`、`R15-AI-02`、`R15-H1-04`、`R5-AI-01`、`R5-AI-02`、`R5-AI-03`、`R5-AI-04` |
...
- `R1.5` 的 AI gate 若在发版前沿用当前 provider / model，还必须额外复跑 [24-single-classify-h1-replay-validation.md](./24-single-classify-h1-replay-validation.md) 对应脚本，确认 `/api/ai/test` 和单条 `/api/ai/classify` 不再超时。
- `R2` 的最终 gate 以 `npm test`、`npm run build`、[11-playwright-mcp-release-journeys.md](./11-playwright-mcp-release-journeys.md)、[12-extension-roundtrip-validation.md](./12-extension-roundtrip-validation.md)、[19-extension-runtime-validation.md](./19-extension-runtime-validation.md)、[20-extension-action-popup-validation.md](./20-extension-action-popup-validation.md)、[10-ai-provider-h1-validation.md](./10-ai-provider-h1-validation.md)、[24-single-classify-h1-replay-validation.md](./24-single-classify-h1-replay-validation.md) 和 [13-release-handoff.md](./13-release-handoff.md) 为准。
...
| AI classify / test / classify-batch | `tests/ai-classify-guardrail.test.ts`、`tests/ai-classify-semantic-samples.test.ts`、`tests/integration/ai-routes.test.ts`、`tests/integration/ai-harness.test.ts` + `scripts/ai-classify-semantic-validate.ts` + `scripts/ai-h1-classify-semantic-validate.ts` + H1 实测 | 本地合同已纳入回归；当前 provider focused H1 replay 未通过 | 单条 `/api/ai/classify` 已同时具备 taxonomy guardrail、常见内容信号语义择优和固定语义样本集；但截至 2026-03-30，当前本地 provider 在 `/api/ai/test` 和单条 classify 上都出现了 timeout，需要先处理 provider 可用性再把 real-provider gate 视为绿色。 |
| AI classify 输出合同 / 语义择优 | `src/ai-classify-guardrail.ts` + [18-single-classify-guardrail-validation.md](./18-single-classify-guardrail-validation.md) + [22-single-classify-semantic-validation.md](./22-single-classify-semantic-validation.md) + [23-single-classify-sample-gate-validation.md](./23-single-classify-sample-gate-validation.md) + [24-single-classify-h1-replay-validation.md](./24-single-classify-h1-replay-validation.md) | 本地 contract 已纳入回归 | 单条 `/api/ai/classify` 现在会被强制收口到当前模板 / 分类树；在文档、教程、示例、GitHub issues/releases、技术社区 host 和浏览器插件商店 host 高信号场景下，还会做本地 deterministic rerank。当前剩余主要是 provider timeout，而不是 taxonomy / semantic rescue 缺口。 |

...
| 项目 | 当前结论 | 证据 |
|---|---|---|
| 构建基线 | `npm run build` 于 2026-03-30 通过。 | 当前工作区在 `R6-EXT-04` 内置扩展 popup UI 收口后重新 clean run 通过。 |
| 自动化测试基线 | `npm test` 于 2026-03-30 通过，`22` 个测试文件、`169` 条测试全部通过。 | 当前工作区在 `R6-EXT-04` 内置扩展 popup UI 收口后重新 clean run 通过。 |
...
| 浏览器扩展 round-trip | `extension-new/` | `tests/extension-popup-shell.test.ts`、`scripts/extension-roundtrip-validate.ts`、`scripts/extension-runtime-validate.ts`、`scripts/extension-action-popup-validate.ts` | 无需额外人工 gate；真实运行时与 action popup 目标都可复跑，popup UI 主链路也已有 runtime 验收 | 剩余缺口主要转入 SingleFile 注入、超时、重复点击与失败恢复稳健性 | `R2-EXT-02`、`R5-EXT-02`、`R5-EXT-03`、`R6-EXT-04`、`R6-EXT-05` |
...
| 内置扩展 popup UI | `tests/extension-popup-shell.test.ts` + `scripts/extension-runtime-validate.ts` + [33-extension-popup-ui-validation.md](./33-extension-popup-ui-validation.md) | 已纳入回归 | 当前已验证 popup 的主操作层级、设置区摘要、成功 / 失败 / loading 状态和按钮恢复；剩余扩展工作主要转入 `R6-EXT-05` 的 SingleFile 稳健性。 |

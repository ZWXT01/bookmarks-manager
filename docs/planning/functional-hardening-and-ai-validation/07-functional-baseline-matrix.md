...
| 项目 | 当前结论 | 证据 |
|---|---|---|
| 构建基线 | `npm run build` 于 2026-03-31 通过。 | 当前工作区在 `R7-AI-02` organize 作用域冻结收口后重新 clean run 通过。 |
| 自动化测试基线 | `npm test` 于 2026-03-31 通过，`22` 个测试文件、`179` 条测试全部通过。 | 当前工作区在 `R7-AI-02` organize 作用域冻结收口后重新 clean run 通过。 |
...
| AI organize 生命周期 | `tests/integration/ai-organize-routes.test.ts` + `tests/ai-organize-plan.test.ts` + MCP UI + H1 apply / rollback + [30-organize-apply-contract-validation.md](./30-organize-apply-contract-validation.md) + [36-ai-organize-assigning-lock-validation.md](./36-ai-organize-assigning-lock-validation.md) + [37-ai-organize-scope-freeze-validation.md](./37-ai-organize-scope-freeze-validation.md) | 已纳入回归 | 当前已明确支持同模板不重叠直接 apply、同模板重叠显式 resolve、跨模板隔离 apply；`start / retry` 也已共用 `assigning` 单活锁，cancel 中的旧 plan 不会再把 stale preview 写回，retry 也不会再回扫 live scope 把后续新增书签吸进原计划。真实 provider 质量仍以“可解释、可回退”为准，不承诺零误判。 |

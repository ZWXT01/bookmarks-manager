...
| 项目 | 当前结论 | 证据 |
|---|---|---|
| 构建基线 | `npm run build` 于 2026-03-31 通过。 | 当前工作区在 `R7-AI-03` organize retry 预检与旧失败产物清理收口后重新 clean run 通过。 |
| 自动化测试基线 | `npm test` 于 2026-03-31 通过，`22` 个测试文件、`182` 条测试全部通过。 | 当前工作区在 `R7-AI-03` organize retry 预检与旧失败产物清理收口后重新 clean run 通过。 |
...
| 设置页 | `src/routes/settings.ts`、`views/settings.ejs`、`scripts/settings-ai-diagnostic-validate.ts` | `tests/integration/ops-routes.test.ts`、`tests/integration/page-assets.test.ts` | 设置页 AI 测试成功态和诊断失败态已有浏览器 harness | 其余设置项仍主要依赖 API 合同和基础页面渲染，不含更多浏览器交互回归 | `R1-API-02`、`R1-QA-01`、`R5-AI-08` |
| 模板 | `src/routes/templates.ts`、`src/template-service.ts`、`views/index.ejs`、`public/app.js` | `tests/integration/ops-routes.test.ts`、`tests/integration/page-assets.test.ts` | `scripts/category-interaction-validate.ts` 已覆盖模板切换；`scripts/template-editor-validate.ts` 已覆盖长树编辑与 footer 可达性 | 剩余缺口主要在“扩充更多预置模板并验证切换主链路” | `R1-API-02`、`R6-UI-02`、`R6-AI-03`、`R6-TPL-06` |

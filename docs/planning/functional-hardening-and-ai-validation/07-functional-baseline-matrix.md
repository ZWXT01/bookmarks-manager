...
| AI organize 生命周期 | `src/routes/ai.ts`、`src/ai-organize.ts`、`src/ai-organize-plan.ts`、`src/routes/pages.ts` | `tests/ai-organize-plan.test.ts`、`tests/integration/ai-organize-routes.test.ts`、`tests/integration/ai-harness.test.ts` | 任务详情页可展示 organize 计划；MCP UI 与真实 H1 已覆盖 apply / rollback；`R6-AI-03` 又补齐了模板编辑后的默认入口与显式模板隔离合同 | 多待应用 plan 合同和模板编辑后的默认模板源都已收口；当前残余主要回到 provider 语义准确率与后续模板扩容，而不是 plan 读错模板 | `R15-AI-03`、`R3-AI-02`、`R6-AI-01`、`R6-AI-03` |
...
| AI organize 生命周期 | `tests/integration/ai-organize-routes.test.ts` + MCP UI + H1 apply / rollback + [30-organize-apply-contract-validation.md](./30-organize-apply-contract-validation.md) | 已纳入回归 | 当前已明确支持同模板不重叠直接 apply、同模板重叠显式 resolve、跨模板隔离 apply；真实 provider 质量仍以“可解释、可回退”为准，不承诺零误判。 |

...
| 项目 | 当前结论 | 证据 |
|---|---|---|
| 构建基线 | `npm run build` 于 2026-03-30 通过。 | 当前工作区在 `R5-AI-07` provider diagnose 收口后执行通过。 |
| 自动化测试基线 | `npm test` 于 2026-03-30 通过，`19` 个测试文件、`153` 条测试全部通过。 | 当前工作区在 `R5-AI-07` provider diagnose 收口后执行通过。 |
...
| AI classify / test / classify-batch | `src/routes/ai.ts`、`src/ai-classify-guardrail.ts`、`scripts/ai-classify-semantic-validate.ts`、`scripts/ai-h1-classify-semantic-validate.ts`、`scripts/ai-provider-diagnose.ts` | `tests/ai-classify-guardrail.test.ts`、`tests/ai-classify-semantic-samples.test.ts`、`tests/integration/ai-routes.test.ts`、`tests/integration/ai-harness.test.ts` | `R15-H1-04` 已做历史真实 provider 验收；`R5-AI-04` 又补了 focused H1 replay；`R5-AI-05` 再补单条 classify timeout fallback；`R5-AI-06` 再补 `/api/ai/test` 瞬时重试；`R5-AI-07` 再补 direct diagnose 与 timeout diagnostic payload | 输出合同、常见语义漂移和固定语义样本集都已收口；当前剩余主要缺口已精确收敛到 real provider 的 chat completion 可用性 / timeout 稳定性，而不是配置探活本身 | `R15-AI-01`、`R15-AI-02`、`R15-H1-04`、`R5-AI-01`、`R5-AI-02`、`R5-AI-03`、`R5-AI-04`、`R5-AI-05`、`R5-AI-06`、`R5-AI-07` |
...
- `R1.5` 的 AI gate 若在发版前沿用当前 provider / model，还必须额外复跑 [24-single-classify-h1-replay-validation.md](./24-single-classify-h1-replay-validation.md)、[25-single-classify-timeout-fallback-validation.md](./25-single-classify-timeout-fallback-validation.md)、[26-ai-test-retry-validation.md](./26-ai-test-retry-validation.md) 和 [27-ai-provider-diagnostic-validation.md](./27-ai-provider-diagnostic-validation.md) 对应脚本 / 结论，确认 `/api/ai/test` 状态、单条 classify fallback 和 direct diagnose 都在预期内。
- `R2` 的最终 gate 以 `npm test`、`npm run build`、[11-playwright-mcp-release-journeys.md](./11-playwright-mcp-release-journeys.md)、[12-extension-roundtrip-validation.md](./12-extension-roundtrip-validation.md)、[19-extension-runtime-validation.md](./19-extension-runtime-validation.md)、[20-extension-action-popup-validation.md](./20-extension-action-popup-validation.md)、[10-ai-provider-h1-validation.md](./10-ai-provider-h1-validation.md)、[24-single-classify-h1-replay-validation.md](./24-single-classify-h1-replay-validation.md)、[25-single-classify-timeout-fallback-validation.md](./25-single-classify-timeout-fallback-validation.md)、[26-ai-test-retry-validation.md](./26-ai-test-retry-validation.md)、[27-ai-provider-diagnostic-validation.md](./27-ai-provider-diagnostic-validation.md) 和 [13-release-handoff.md](./13-release-handoff.md) 为准。
...
| AI classify / test / classify-batch | `tests/ai-classify-guardrail.test.ts`、`tests/ai-classify-semantic-samples.test.ts`、`tests/integration/ai-routes.test.ts`、`tests/integration/ai-harness.test.ts` + `scripts/ai-classify-semantic-validate.ts` + `scripts/ai-h1-classify-semantic-validate.ts` + `scripts/ai-provider-diagnose.ts` + H1 实测 | 本地合同已纳入回归；单条 classify 的 timeout fallback 已在真实 provider 下恢复 `1/1` focused replay；`/api/ai/test` 现在还能在失败时给出 `/models` 级别诊断 | 单条 `/api/ai/classify` 已同时具备 taxonomy guardrail、常见内容信号语义择优、固定语义样本集和 timeout fallback；截至 2026-03-30，当前本地 provider 的主要残余已进一步收敛到 chat completion 链路的可用性，而不是单条 classify 高信号输入的完全不可用，也不是 base_url / model 配置本身不可达。 |

...
| `RISK-017` | 2026-03-30 | `R6-AI-01` | Organize apply contract | open | todo | `R6-AI-01` |  |  |  |  | 多个 `preview` organize plan 同时存在时，当前是否“只允许最新 plan 应用”并不直观；实际行为还取决于模板和书签集合是否重叠，操作者很容易误判可应用范围。 | 当前 `R3-AI-02` 只把同模板且书签集合重叠的旧 plan 收口为 `409 stale`，并靠 `source_snapshot` 防止脏写；但这层规则主要藏在后端实现里，页面和台账还没有把“同模板重叠 / 不重叠 / 跨模板”的合同显式说清楚。 | 需要把最终合同收口到“同模板不重叠可直接 apply、跨模板可隔离 apply、同模板重叠可进入显式冲突解决”，并补页面提示和自动化 gate。 |

...
| `RISK-011` | 2026-03-29 | `R3-UI-01` | Frontend UI stability | open | todo | `R3-UI-01` |  |  |  |  | 首页分类导航出现“初始水平、刷新后转为垂直”的布局漂移，且滚动 / 拖拽失效时会导致部分分类不可达，说明前端 hydration / layout 仍可能存在系统性回归。 | 当前只有人工发现和口头反馈，缺少正式回归；已计划由 `R3-UI-01` 集中收口首页分类导航和同类 UI 布局漂移。 | 优先建立稳定复现步骤，并把刷新前后布局一致性、分类可达性纳入可复跑 UI 验收。 |
| `RISK-012` | 2026-03-29 | `R3-AI-02` | AI organize data safety | open | todo | `R3-AI-02` |  |  |  |  | AI organize 会改动书签与分类；若在 plan 生成后用户又移动、编辑、删除书签或分类，或多个 organize 任务交错 apply，当前仍可能出现误应用、脏写或数据丢失风险。 | `R15-AI-03` 已覆盖基础 apply / rollback / stale recovery，但尚未系统覆盖多 plan 时序、书签 / 分类被并发修改等高风险组合；已计划由 `R3-AI-02` 继续补强。 | 明确 multi-plan stale / invalid contract，并增加数据安全 guardrail 与高风险回归用例。 |
| `RISK-013` | 2026-03-29 | `R3-QA-03` | Cross-view interaction consistency | open | todo | `R3-QA-03` |  |  |  |  | 分类管理页排序、首页分类导航顺序、分类下拉 / 筛选等跨页面状态可能不一致；拖拽、移动或删除后如果不同步，用户会看到错误导航或产生误操作。 | 当前已有 API 与单页 smoke，但缺少跨页面交互一致性回归；已计划由 `R3-QA-03` 集中补齐。 | 至少覆盖排序同步、拖拽后同步、刷新后保持一致，以及相似的模板 / 书签移动联动。 |

...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `RISK-001` | 2026-03-28 | `R15-AI-01` | AI provider contract | mitigated | done | `R15-AI-01,R5-AI-01,R5-AI-02,R5-AI-03,R5-AI-04,R5-AI-05,R5-AI-06,R5-AI-07,R5-AI-08` | `061bb0f,c3ad87c,49515ec,b457d50,1b98512,e7face9,8dfffca,c44f362,d9c24ef` | 2026-03-28 20:50:01 +0800 | 2026-03-30 03:25:03 +0800 |  | 真实 OpenAI 兼容模型可能不支持“联网浏览”或稳定 JSON 输出，导致 `classify` / `organize` 质量和回归结果不可重复。 | `R15-AI-01` 已引入可注入 AI client factory、队列式 fixture harness；`R15-H1-04` 又用固定 3 条书签样本完成真实 provider 验收，确认 `/api/ai/test`、`classify-batch`、`organize`、`apply/rollback` 均可用。`R5-AI-01` 把单条 `/api/ai/classify` 的输出强制收口到当前模板 / 分类树内；`R5-AI-02` 又基于 `title`、`url`、可选 `description`、常见 host/path 信号和分类别名补了本地 deterministic 语义择优；`R5-AI-03` 则进一步固化了覆盖文档、教程、课程、代码示例、GitHub issues/releases、技术社区 host 和浏览器插件商店 host 的固定样本集，并提供 `scripts/ai-classify-semantic-validate.ts` 作为复验入口。`R5-AI-04` 又新增 `scripts/ai-h1-classify-semantic-validate.ts`，把 focused H1 replay 收口到固定语义样本、真实路由 timeout、`--ids` / `--skip-test` / `--retries` 与自动清理；`2026-03-30` 首次 replay 证明当前本地 provider 在 `/api/ai/test` 与 `react-reference-docs` 的单条 `/api/ai/classify` 上都会 timeout。`R5-AI-05` 继续把单条 classify 的 timeout / 连接型故障收口成 deterministic fallback，并用 focused H1 replay 证明 `react-reference-docs` 现在可在 `--skip-test` 条件下回到 `200` 和 `学习资源/官方文档`。`R5-AI-06` 又为 `/api/ai/test` 增加了 1 次仅针对瞬时连接型故障的重试，本地合同已证明短暂 timeout 可恢复，但 focused H1 replay 仍停在 `/api/ai/test -> 500 Request timed out.`。`R5-AI-07` 则继续为 `/api/ai/test` 增加 `/models` 诊断分支，并新增 `scripts/ai-provider-diagnose.ts`：当前本地 provider 已明确表现为“`/models` 200、配置 model 存在，但 `/chat/completions` 在 30s timeout 窗口内仍失败”。`R5-AI-08` 又把这组诊断直接接到了设置页 UI，并新增 `scripts/settings-ai-diagnostic-validate.ts` 证明操作员现在能在浏览器里直接看到“基础连通正常，聊天补全未通过”和对应的 `models` 诊断细节。 | 当前残余已进一步收敛为“当前 provider / model 的聊天补全链路不可用或显著不稳定”，而不是“配置不通”或“诊断信息只有开发者能看到”。后续若模板明显变更、provider / model 切换或发版前要复核 AI，应先复跑 `scripts/ai-provider-diagnose.ts`、`scripts/ai-h1-classify-semantic-validate.ts` 与 `scripts/settings-ai-diagnostic-validate.ts`；若 `/api/ai/test` 返回 `models_ok=true` 且 `model_found=true`，就优先处理 provider 侧 chat completion 可用性。单条 classify 的高信号输入现在已有 timeout fallback，但 `/api/ai/test` 与更广泛的 provider 健康状况仍不应被当成无条件绿色。 |

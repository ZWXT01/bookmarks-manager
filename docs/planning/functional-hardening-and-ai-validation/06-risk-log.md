...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `RISK-001` | 2026-03-28 | `R15-AI-01` | AI provider contract | mitigated | done | `R15-AI-01,R5-AI-01,R5-AI-02,R5-AI-03,R5-AI-04` | `061bb0f,c3ad87c,49515ec,b457d50,1b98512` | 2026-03-28 20:50:01 +0800 | 2026-03-30 01:43:08 +0800 |  | 真实 OpenAI 兼容模型可能不支持“联网浏览”或稳定 JSON 输出，导致 `classify` / `organize` 质量和回归结果不可重复。 | `R15-AI-01` 已引入可注入 AI client factory、队列式 fixture harness；`R15-H1-04` 又用固定 3 条书签样本完成真实 provider 验收，确认 `/api/ai/test`、`classify-batch`、`organize`、`apply/rollback` 均可用。`R5-AI-01` 把单条 `/api/ai/classify` 的输出强制收口到当前模板 / 分类树内；`R5-AI-02` 又基于 `title`、`url`、可选 `description`、常见 host/path 信号和分类别名补了本地 deterministic 语义择优；`R5-AI-03` 则进一步固化了覆盖文档、教程、课程、代码示例、GitHub issues/releases、技术社区 host 和浏览器插件商店 host 的固定样本集，并提供 `scripts/ai-classify-semantic-validate.ts` 作为复验入口。`R5-AI-04` 又新增 `scripts/ai-h1-classify-semantic-validate.ts`，把 focused H1 replay 收口到固定语义样本、真实路由 timeout、`--ids` / `--skip-test` / `--retries` 与自动清理；`2026-03-30` 对当前本地 provider 的 focused replay 结果表明，`/api/ai/test` 与 `react-reference-docs` 的单条 `/api/ai/classify` 在真实 `30s / 60s` 合同下都返回了 `500 Request timed out.`。 | 当前残余已从“taxonomy / semantic guardrail 不足”进一步收敛为“当前 provider / model 的可用性与时延不稳定”。后续若模板明显变更、provider / model 切换或发版前要复核 AI，应先复跑 `scripts/ai-h1-classify-semantic-validate.ts`；若 `/api/ai/test` 或单条 `/api/ai/classify` 继续超时，就先处理 provider 侧可用性，不要把真实 provider gate 当成绿色。 |

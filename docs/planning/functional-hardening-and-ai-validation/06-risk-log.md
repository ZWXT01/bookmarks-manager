...
| `RISK-018` | 2026-03-30 | `R6-UI-02` | Template editor modal accessibility | open | todo | `R6-UI-02` |  |  |  |  | 自定义模板分类项不断增多时，模板编辑弹窗会持续拉长，长树编辑下保存 / 取消等关键操作可能被挤出可视区，尤其在小视口或软键盘场景更明显。 | 当前模板编辑弹窗虽然已有 `max-h-[85vh]` 和内部滚动区，但还缺少对长树、小屏和操作条可达性的稳定保障，也没有针对这类长列表编辑的浏览器级回归。 | 需要把模板编辑弹窗的高度、滚动区和 footer 可达性收口成明确 UI 合同，并补可复跑验收。 |

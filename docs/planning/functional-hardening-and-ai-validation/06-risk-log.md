...
| `RISK-019` | 2026-03-30 | `R6-AI-03` | AI template source drift | open | todo | `R6-AI-03` |  |  |  |  | 模板编辑后，AI 分类入口如果仍读取旧模板树、旧候选分类或陈旧 snapshot，就会出现“界面模板已变、AI 仍按旧分类分类”的隐蔽漂移。 | 当前单条 `/api/ai/classify` 已优先读取最新活动模板；`organize` 也支持显式 `template_id`。但“模板编辑并重新应用后，所有默认 AI 入口都跟随最新活动模板”还缺少系统性合同说明和端到端回归。 | 需要把“默认入口读最新活动模板、显式 `template_id` 读指定模板”的合同写实，并对模板增删改查后的 AI 候选分类刷新建立回归。 |
| `RISK-020` | 2026-03-30 | `R6-EXT-04` | Built-in extension UI debt | open | todo | `R6-EXT-04` |  |  |  |  | 项目内置扩展目前已覆盖功能合同，但 popup 仍偏工程化：主操作、设置区、分类选择和状态反馈的层次感不足，影响可读性和使用体验。 | `R5-EXT-02`、`R5-EXT-03` 已覆盖真实 runtime 和 action popup 合同，证明扩展能工作；但现有验收更偏功能正确性，没有收口 popup 的视觉与交互质量。 | 需要单独立项处理 popup UI 美化、信息架构和交互提示，避免“能用但不好用”长期留在 release 面。 |
| `RISK-021` | 2026-03-30 | `R6-EXT-05` | Built-in extension SingleFile robustness | open | todo | `R6-EXT-05` |  |  |  |  | 内置扩展基于 SingleFile 做快照，注入失败、页面不可连接、重复点击、长页面超时或大内容上传时，仍可能出现状态恢复不稳、错误提示粗糙或资源开销偏大的问题。 | 当前已有 popup-harness、真实 runtime 和 action popup 验收，覆盖了主成功链路和部分失败提示；但对 SingleFile 注入、重复触发、超时恢复和性能抖动还没有单独的稳健性 gate。 | 需要进一步优化 `popup.js` / `content.js` 的 SingleFile 逻辑，并补关键失败路径的真实 runtime 回归。 |
| `RISK-022` | 2026-03-30 | `R6-TPL-06` | Preset template coverage debt | open | todo | `R6-TPL-06` |  |  |  |  | 当前内置预置模板数量偏少，用户一旦组织场景偏离默认模板，就容易被迫从零编辑长树模板，放大模板编辑成本和 AI 分类 taxonomy 不贴合的问题。 | 现有模板系统、活动模板切换和 AI 默认入口已经可用，但预置模板集合仍过窄，缺少对不同信息组织场景的开箱即用支持，也缺少针对模板切换主链路的专项回归。 | 需要新增多套预置模板，并把“创建预置模板 -> 应用 -> 首页 / 管理页 / AI 默认分类入口同步更新”的链路纳入回归。 |

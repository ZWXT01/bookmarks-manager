...
| `RISK-006` | 2026-03-28 | `R15-AI-03` | AI settings drift | resolved | done | `R15-AI-03` |  | 2026-03-29 00:19:38 +0800 | 2026-03-29 00:45:20 +0800 |  | 设置页存有 `ai_batch_size`，但 AI 路由默认使用请求体 `batch_size`，导致配置含义不一致。 | `R15-AI-03` 已将 `ai_batch_size` 标准化为运行时权威默认值，并用 classify-batch、organize、retry、settings reset 的自动化用例证明配置实际生效。 | 在 `R15-H1-04` 只继续验证真实 provider 质量，不再把 batch-size 配置合同视为主要风险。 |

...
| risk_id | 状态 | 说明 |
|---|---|---|
| `RISK-001` | mitigated | 单条 `/api/ai/classify` 现在已具备 taxonomy guardrail + 本地语义择优 + timeout fallback；`/api/ai/test` 也已具备 1 次瞬时重试和 `/models` 诊断，但当前本地 provider 仍未恢复绿色，且直连 probe 已证明主要问题集中在 chat completion 链路，发版前仍需复跑或更换稳定 provider。 |

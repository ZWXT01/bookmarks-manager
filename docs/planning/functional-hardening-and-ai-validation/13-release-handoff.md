...
| 项目 | 证据 | 结果 |
|---|---|---|
| 自动化测试 | `npm test` 于 2026-04-01 通过，`22` 个测试文件、`188` 条测试全部通过。 | 通过 |
...
| 历史 issue Playwright 浏览器复验 | [42-playwright-historical-issue-regression-validation.md](./42-playwright-historical-issue-regression-validation.md) 已 clean run `10` 条浏览器脚本，补齐 `R1/R2/R3/R4/R5/R6/R7` 历史 issue 的统一浏览器回放入口。 | 通过 |
...
- UI gate 仍以内置 Playwright MCP 为主；`R7-QA-07` 新增的 `scripts/playwright-issue-regression-validate.ts` 是历史 issue 的补充 browser replay，不等于恢复仓库内 `e2e/` 和 `playwright.config.ts` 为主 gate。

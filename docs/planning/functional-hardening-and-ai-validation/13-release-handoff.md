...
| 单条 classify focused H1 replay | [24-single-classify-h1-replay-validation.md](./24-single-classify-h1-replay-validation.md) 已对当前本地 provider 做 focused replay；`/api/ai/test` 与单条 `/api/ai/classify` 都返回 `Request timed out.`。 | 未通过 |
...
- 截至 `2026-03-30`，当前本地 provider 的 focused H1 replay 未通过，因此 real-provider gate 不能再被视为“无条件绿色”；若沿用当前 provider / model 发版，必须先解决 timeout 或重新跑通 focused replay。

## 4. 保留风险

| risk_id | 状态 | 说明 |
|---|---|---|
| `RISK-001` | mitigated | 单条 `/api/ai/classify` 现在已具备 taxonomy guardrail + 本地语义择优；但 `2026-03-30` 的 focused H1 replay 表明当前本地 provider 在 `/api/ai/test` 和单条 classify 上都会 timeout，发版前仍需复跑或更换稳定 provider。 |

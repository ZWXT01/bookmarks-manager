...
| `/api/ai/test` 瞬时重试 | [26-ai-test-retry-validation.md](./26-ai-test-retry-validation.md) 已证明本地 timeout-retry 合同成立，但当前本地 provider 的 focused H1 replay 仍停在 `500 Request timed out.`。 | 部分通过 |
...
- 截至 `2026-03-30`，当前本地 provider 的 `/api/ai/test` 即使加了 1 次瞬时重试也仍不是稳定绿色；但单条 `/api/ai/classify` 对高信号输入已具备 timeout fallback，用户面风险已进一步收口。

## 4. 保留风险

| risk_id | 状态 | 说明 |
|---|---|---|
| `RISK-001` | mitigated | 单条 `/api/ai/classify` 现在已具备 taxonomy guardrail + 本地语义择优 + timeout fallback；`/api/ai/test` 也已加 1 次瞬时重试，但当前本地 provider 仍未恢复绿色，发版前仍需复跑或更换稳定 provider。 |

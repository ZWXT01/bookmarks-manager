...
| 60 | dev | W2 | UIR-DEV-060 | 首页信息架构与导航壳体重构 | 重组首页主工作区，让分类导航、侧边工具区和内容区层级更清晰 | `views/index.ejs` 主布局；顶部 header、侧边工具区、分类 tabs、子分类面板、移动端 drawer | 不改首页数据来源与分类 API | A0 | UIR-DEV-010,UIR-DEV-050 | `04-change-regression-log.md#uir-dev-060` | 首页桌面 / 移动壳体无遮挡 / 溢出 / 滚动冲突；分类导航更易达；现有 tab selector 保留 | in_progress | failed | 2026-04-17 |  | 本地 page-assets 与 category-nav harness 已通过；`scripts/category-interaction-validate.ts` 暴露 table 行动作菜单在新壳体下的层级/裁切问题，需在进入 live verify 前继续收口 |
